...
describe("readSyncSnapshot RPC (SSR read path, ADR-0011 D1)", () => {
  ...
  it("normalizes BLOB values to Uint8Array — parity with the wire codec (ADR-0017)", async () => {
    const room = `snap-blob-${crypto.randomUUID()}`
    await runInDurableObject(stubFor(room), (_i, s) => {
      // SQLite affinity doesn't coerce blobs: a TEXT column stores this as BLOB
      // and reads it back as ArrayBuffer — exactly what RPC structured clone
      // would leak through where the WS codec normalizes to Uint8Array.
      s.storage.sql.exec("INSERT INTO messages(id,body) VALUES('a', ?)", new Uint8Array([1, 2, 3]))
    })
    const { rows } = await readSyncSnapshot(room, { collection: "messages" })
    expect(rows[0]!.body).toBeInstanceOf(Uint8Array)
    expect(Array.from(rows[0]!.body as Uint8Array)).toEqual([1, 2, 3])
  })
