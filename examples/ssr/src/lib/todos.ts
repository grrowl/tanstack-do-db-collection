...
export function todosOptions(transport: Transport<TodosApi>) {
  return collectionOptions(
    doCollectionOptions({ transport, table: "todos", getKey: (t) => t.id }) as never,
  ) as never
}
...
export function inertSsrTransport(): Transport<TodosApi> {
  return new SsrSnapshotTransport<TodosApi>({ read: () => new Promise(() => {}) })
}
